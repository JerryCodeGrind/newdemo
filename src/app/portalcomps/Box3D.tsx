...
const Box3D = ({ initialPosition = [0, 0, -30], onZoomStart, onBoxClicked, getTextAnimationControls }: Box3DProps) => {
  ...
  const startBoxAnimation = useCallback(() => {
    ...
    // Fade out navigation and title text immediately
    const navElement = document.querySelector('nav');
    const titleElement = document.querySelector('.hero-title-container');
  ...
  }, [camera, router, onZoomStart, getTextAnimationControls, scene]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('triggerBlueboxAnimation', eventHandler);
    return () => {
      window.removeEventListener('triggerBlueboxAnimation', eventHandler);
    };
  }, [startBoxAnimation]);
